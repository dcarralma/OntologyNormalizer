...
public class ABoxNormalizer {
	...
	public static void normalizeClassAsss(Set<OWLClassAssertionAxiom> classAsss, Set<OWLSubClassOfAxiom> subClassOfAxioms) {
		...
		for (OWLClassAssertionAxiom classAss : classAssCopy) {
			...
			if (classExpr.getClassExpressionType().equals(ClassExpressionType.OWL_CLASS))
				...
				// C(a) -> { X(a), X sqs C }
				subClassOfAxioms.add(new OWLSubClassOfAxiomImpl(Srd.getCorrespondingFreshClass(classExpr), classExpr, new HashSet<OWLAnnotation>()));
				classAsss.add(new OWLClassAssertionAxiomImpl(classAss.getIndividual(), Srd.getCorrespondingFreshClass(classExpr), new HashSet<OWLAnnotation>()));
			}
		}
	}
	...
	public static void normalizeNegativeObjPropAsss(Set<OWLNegativeObjectPropertyAssertionAxiom> negativeObjPropAssertions, Set<OWLObjectPropertyAssertionAxiom> objPropAss,
		...
		for (OWLNegativeObjectPropertyAssertionAxiom negativeObjPropAss : negativeObjPropAssertions) {
			...
			disjointPropAx.add(new OWLDisjointObjectPropertiesAxiomImpl(Srd.toSet(Srd.getCorrespondingFreshObjProp(objProp), objProp), new HashSet<OWLAnnotation>()));
			objPropAss.add(
				new OWLObjectPropertyAssertionAxiomImpl(negativeObjPropAss.getSubject(), Srd.getCorrespondingFreshObjProp(objProp), negativeObjPropAss.getObject(), new HashSet<OWLAnnotation>()));
		}
	}
